CREATE OR REPLACE FUNCTION public.submit_student_exam(p_exam_id uuid, p_student_name text, p_student_class text, p_answers jsonb, p_violations jsonb DEFAULT '[]'::jsonb, p_violation_count integer DEFAULT 0, p_started_at timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
  ...
  FOR v_q IN SELECT jsonb_array_elements(COALESCE(v_qs->'partII', '[]'::jsonb)) LOOP
    ...
    FOR v_item IN SELECT jsonb_array_elements(COALESCE(v_q->'items', '[]'::jsonb)) LOOP
      ...
      IF jsonb_typeof(v_chosen) = 'array' THEN
        -- legacy: mảng các key được chọn là Đúng
