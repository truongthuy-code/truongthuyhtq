ALTER TABLE public.submissions ADD COLUMN IF NOT EXISTS duration_seconds integer;

CREATE OR REPLACE FUNCTION public.submit_student_exam(p_exam_id uuid, p_student_name text, p_student_class text, p_answers jsonb, p_violations jsonb DEFAULT '[]'::jsonb, p_violation_count integer DEFAULT 0, p_started_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_duration_seconds integer DEFAULT NULL)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.submissions(
    ...
    correct_count, wrong_count, violation_count, violations, started_at, duration_seconds
  )
  VALUES (
